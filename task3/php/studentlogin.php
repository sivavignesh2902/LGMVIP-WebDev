...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="../css/studentlogin.css?v=<?php echo time(); ?>">
    <!-- Bootstrap CSS -->
...
<div class="heading">
    ...
    <input type="submit" class="btn btn-success col-sm-5 offset-3"  value="Login" name="stu_login"  />
        </div>
</form>
</body>
</html>
<?php
    if(isset($_POST['stu_login'])){
        ...
        $check_credential = "SELECT COUNT(*) FROM students WHERE username='{$uname}' AND password = '{$pwd}' ";
        $process_check_credential = mysqli_query($dbconn,$check_credential);
        if(intval($process_check_credential) == 1){
            $_SESSION['stu_username'] = $uname;
            echo "<script>
